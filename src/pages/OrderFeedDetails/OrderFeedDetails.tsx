...
const OrderFeedDetails = () => {
  ...
  const groupedIngedientsView = useMemo(() => groupedIngedients.map((ingredient) => (
    <div key={ingredient._id}  className={` mt-4 ${styles.ingredient}`}>
      <div className={`text text_type_main-default ${styles.leftIngredientRow} `}>
          ...
          <div key={ingredient._id} className={`${styles.imgContainer} `}>
            <div>
              <picture className={styles.picture}>
                ...
                <img src={ingredient.image} alt={ingredient.name} width="112" height="56" />
              </picture>
            </div>
          </div>
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.order}>
        ...
        <div className={`text text_type_main-small mt-3 ${order?.status === 'Выполнен' ? styles.status_done : ''}`}>
          {order?.status}
        </div>
